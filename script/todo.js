...
function loadTasks() {
  ...
  tasks.forEach((task) => {
    ...
    li.innerHTML = `<button onclick="taskComplete(this)" id="btn" class="check" ${
...
function addTask() {
  ...
  /* List */
  const li = document.createElement("li");
  li.innerHTML = `<button onclick="taskComplete(this)" class="check" ${
    task.completed ? "checked" : ""
  }>✔</button>
      <input type="text" value="${
        task.value
      }" class="task" onfocus="getCurrentTask(this)" onblur="editTask(this)">
  <button class="cross-button" onclick="removeTask(this)">✖</button>`;
  list.insertBefore(li, list.children[0]);
  task.value = "";
}
  ...
  tasks.forEach((task) => {
    if (
      confirm(
        ...
        task.task === event.parentNode.children[1].value
      )
    ) {
